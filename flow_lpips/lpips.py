...
class LPIPS(nn.Module):
    def __init__(
        ...
        if self.pnet_type in ["vgg", "vgg16"]:
            net_type = networks.vgg16
            self.chns = [64, 128, 256, 512, 512]
        elif self.pnet_type == "alex":
            net_type = networks.alexnet
    ...
    def forward(self, in0, in1, retPerLayer=False, normalize=False):
        ...
        for kk in range(self.L):
            feats0[kk], feats1[kk] = (
                utils.normalize_tensor(outs0[kk]),
...
class ScalingLayer(nn.Module):
    def __init__(self):
        ...
        self.register_buffer(
            "shift", flow.Tensor([-0.030, -0.088, -0.188])[None, :, None, None]
        )
        self.register_buffer(
            "scale", flow.Tensor([0.458, 0.448, 0.450])[None, :, None, None]
        )
...
class Dist2LogitLayer(nn.Module):
    ...
    def forward(self, d0, d1, eps=0.1):
        return self.model.forward(
            flow.cat((d0, d1, d0 - d1, d0 / (d1 + eps), d1 / (d0 + eps)), dim=1)
        )
...
class L2Norm(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            ...
            value = flow.mean(
                flow.mean(
                    flow.mean((in0 - in1) ** 2, dim=1).view(N, 1, X, Y), dim=2
                ).view(N, 1, 1, Y),
                dim=3,
            ).view(N)
            return value
